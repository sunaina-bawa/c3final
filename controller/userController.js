...
const {createJwtToken}=require("../config/db");
...
const registerUser=async(req,res)=>{
    try {
        ...
        if(name && email && gender && password)
        {
            ...
            res.status(201);
            return res.json({message:"user successful created",status:true});
        }
        else
        {
            res.status(400);
            return res.json({
                message:"incomplete detail provided",
                status:false,
            });
        }
...
const loginUser=async(req,res)=>{
    try {
        ...
        {
            res.status(401);
            return res.json({message:"wrong cre", status:false});
